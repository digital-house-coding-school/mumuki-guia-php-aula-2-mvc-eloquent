...
function redirect($route) {
  ...
  if ($route == "filmes/listar" || $route === "/filmes/listar") {
    ...
    throw new Exception("Depois de atualizar o filme você deve redirecionar para a URL /filmes/listar");
  }
...
class Filme extends Model {
  ...
  public function save() {
    ...
    if (!isset($this->id)) {
      throw new Exception("Você está buscando um filme com o método find?");
    }
    if ($this->id !== $request->secret) {
      throw new Exception("O filme que está sendo armazenado não tem o id que chega na URL");
    }
    
    if (!isset($this->title) || $this->title !== $request->title) {
      throw new Exception("O filme que esta sendo armazenado não tem o título no request");
    }
